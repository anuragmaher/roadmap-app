...
router.post('/items/:itemId/vote', [
  body('email').isEmail().normalizeEmail().withMessage('Please enter a valid email address')
], createVote);

// Remove a vote for an item
router.delete('/items/:itemId/vote', [
  body('email').isEmail().normalizeEmail().withMessage('Please enter a valid email address')
], removeVote);

// Get vote statistics for an item
router.get('/items/:itemId/votes', getItemVotes);

// Check if a user has voted for an item
router.get('/items/:itemId/vote/check', checkUserVote);

// Notify voters (admin only)
router.post('/items/:itemId/notify-voters', auth, notifyVoters);
